<tr id="record-{{ $postid }}">
    ...
    <td class="text-center">
        <a href="{{ route('admin.brinkpraat.edit.files', ['id' => $postid]) }}">
    ...
    <td style="padding-left:3%">
        <form class="form-group" id="brinkpraat_post_form_{{ $postid }}">
            @csrf
    ...
    <script>
        $(document).ready(function() {
            $('#toggle_{{ $postid }}').change(function(event) {
                ...
                var formElement = $("#brinkpraat_post_form_{{ $postid }}")
                var formData = $(formElement).serialize();

                $.ajax({
                    url: "{{ route('admin.brinkpraat.publish.files', ['id' => $postid]) }}",
                    type: 'PUT',
                    data: formData,
                    dataType: 'json',
        ...
        $(document).ready(function() {
            $('#delete_{{ $postid }}').submit(function(event) {
                ...
                $.ajax({
                    url: "{{ route('admin.brinkpraat.delete.files', ['id' => $postid]) }}",
                    type: 'PUT',
                    data: formData,
                    dataType: 'json',
                    success: function(response) {
